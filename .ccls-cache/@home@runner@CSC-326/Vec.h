...
template <typename T> bool Vec<T>::empty() {
  return _size == 0;
}
...
template <typename T> void Vec<T>::insert(int idx, T n) {
  if (idx < 0 || idx > _size) throw "Error! Out of Bounds";
  if (_size == _cap) reserve(_size * 2);
...
template <typename T> Vec<T> operator+(const Vec<T> &a, const Vec<T> &b) {
  if (a.size() != b.size()) throw "Error! Sizes do not match";
  Vec<T> vec(a.size());
